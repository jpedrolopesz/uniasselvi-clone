...
import { createPendingAction, executePendingWrite, type PendingAction } from "@/lib/vitru/pending-action";
...
export function StudyPlannerView({
  ...
  const [selectedStudySlot, setSelectedStudySlot] = useState<FreeSlot | null>(null);
  const [pendingVoiceAction, setPendingVoiceAction] = useState<PendingAction | null>(null);
  ...
  }, [activities, availableStudySlots, previewToReview?.id, selectedIsoDate, snapshotNow, viewMode]);
  ...
  useEffect(() => {
    const handleCommand = (event: Event) => {
      ...
      if (command.type === "list_options") { respond(true, `${availableStudySlots.length} horários livres calculados.`); return; }
      if (command.type === "select_option") {
        const snapshot = buildStudyCalendarSnapshot({ activities, selectedIsoDate, view: viewMode, now: snapshotNow, availableStudySlots });
        const resolution = resolveTarget(command.referencia ?? "", snapshot);
        const slot = availableStudySlots.find(item => `study-slot:${item.date}:${item.startTime}:select` === resolution.actionId);
        if (!slot) { respond(false, resolution.ambiguous ? "Encontrei mais de um horário. Qual deles você quer?" : "Esse horário não está na lista disponível."); return; }
        const args = { optionId: `study-slot:${slot.date}:${slot.startTime}`, ...slot };
        setSelectedStudySlot(slot);
        setPendingVoiceAction(createPendingAction("schedule_study", args, "study-calendar"));
        respond(true, `Proposta: ${slot.date}, das ${slot.startTime} às ${slot.endTime}. Diga confirmar para gravar.`);
        return;
      }
      if (command.type === "confirm_write") {
        if (!selectedStudySlot) { respond(false, "Não há proposta de estudo pendente para confirmar."); return; }
        const args = { optionId: `study-slot:${selectedStudySlot.date}:${selectedStudySlot.startTime}`, ...selectedStudySlot };
        const authorization = executePendingWrite(pendingVoiceAction, args, "study-calendar", () => true);
        setPendingVoiceAction(pendingVoiceAction);
        if (!authorization.ok) { respond(false, "A proposta pendente expirou ou não corresponde ao horário selecionado."); return; }
        void fetch("/api/v1/vitru/study-session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId, slot: selectedStudySlot, authorization: authorization.pending }) })
          .then(async response => ({ response, result: await response.json() }))
          .then(({ response, result }) => { respond(response.ok && result.ok, result.message ?? (response.ok ? "Sessão de estudo gravada." : "O horário deixou de estar disponível.")); if (response.ok) { setSelectedStudySlot(null); setPendingVoiceAction(null); } });
      }
    };
  ...
  }, [activities, availableStudySlots, pendingVoiceAction, selectedIsoDate, selectedStudySlot, snapshotNow, userId, viewMode]);
